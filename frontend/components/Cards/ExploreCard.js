...
class Card extends Component {
  ...
  checkSearchWord() {
    //uses setData to find the correct data based on searchword, filter and sorrt
    if (this.props.word.toLowerCase() === "all") {
      ...
      this.setData(
        this.props.continent + "/" + this.props.word,
        this.props.sortType
      );
    }
  }
  ...
  openDetailedCard(destinationID, popularity) {
    GetData(destinationID, "").then(res =>
  ...
  render() {
    ...
    if (
      this.state.currentSerachWord.toLowerCase() !==
      this.props.word.toLowerCase()
    ) {
      this.setState({ currentSerachWord: this.props.word });
      this.checkSearchWord();
    } else if (
      this.state.currentContinent.toLowerCase() !==
      this.props.continent.toLowerCase()
    ) {
      this.setState({ currentContinent: this.props.continent });
      this.checkSearchWord();
    } else if (
      this.state.currentSort.toLowerCase() !== this.props.sortType.toLowerCase()
    ) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableHighlight
              key={item._id}
              underlayColor="white"
              onPress={() => this.openDetailedCard(item._id, item.popularity)}
            >
        ...
        <MaterialDialog
          ...
          onOk={() => {
            this.addFavourite(
              dataElement._id,
              dataElement.name,
              this.isFavourite(dataElement._id)
            );
          }}
          onCancel={() => {
            this.setState({ visible: false });
